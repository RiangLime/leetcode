...
// 
//
// Related Topics 栈 深度优先搜索 字符串 👍 275 👎 0
...
public class Q0388 {
    public static void main(String[] args) {
        Solution solution = new Q0388().new Solution();
    }
    ...
    class Solution {
        public int lengthLongestPath(String input) {
            ...
            List<Integer> fileDepths = new ArrayList<>();
            int max = 0;
            ...
            for (int i = 0; i < singles.length; i++) {
                ...
                int popTime = stack.size()-ptr;
                for (int j = 0; j < popTime; j++) {
                    stack.pop();
                }
                fileDepths.add(ptr);
                System.out.print("depth:"+ptr+"-");
                stack.add(single);

                String path = stackToPath(stack);
                filePaths.add(path);
