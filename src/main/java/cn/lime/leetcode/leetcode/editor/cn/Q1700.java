...
// 
//
// Related Topics 栈 队列 数组 模拟 👍 159 👎 0
...
public class Q1700 {
    public static void main(String[] args) {
        Solution solution = new Q1700().new Solution();
    ...
    class Solution {
        public int countStudents(int[] students, int[] sandwiches) {
            Queue<Integer> sandwichStack = new ArrayDeque<>();
            for (int sandwich : sandwiches) {
                sandwichStack.add(sandwich);
            }
            Queue<Integer> studentStack = new ArrayDeque<>();
            for (int student : students) {
                studentStack.add(student);
            }

            int cnt = 0;
            while (cnt < studentStack.size() * 2) {
                if (Objects.equals(studentStack.peek(), sandwichStack.peek())) {
                    studentStack.poll();
                    sandwichStack.poll();
                    cnt =0;
                }else {
                    cnt++;
                    int student = studentStack.poll();
                    studentStack.offer(student);
                }
            }
