...
public class Q1909 {
    public static void main(String[] args) {
        Solution solution = new Q1909().new Solution();
        solution.canBeIncreasing(new int[]{2,3,1,2});
    }
    ...
    class Solution {
        public boolean canBeIncreasing(int[] nums) {
            ...
            // 驼峰
            if (p-1 <0 || nums[p-1] < nums[p+1]) return true;
            // 低估
            if (p+2 > nums.length-1 || nums[p+2]>nums[p]) return true;
            return false;
